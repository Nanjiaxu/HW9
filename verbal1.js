...
function decide_response(user_said) {
  var response;
  var play_re = /\u6253\u7535\u8bdd\u7ed9\s(.+)/i;  // creating a regular expression
  ...
  if (play_parse_array && state === "initial") {
    response = "\u597d\u7684\uff0c\u6b63\u5728\u6253\u7535\u8bdd\u7ed9" + play_parse_array[1];
  } else if (user_said.toLowerCase().includes("call") && state === "initial") {
    response = "\u7ed9\u8c01\u6253\u7535\u8bdd";
    state = "calling"
  } else if (user_said.toLowerCase().includes("\u518d\u89c1")) {
    response = "\u518d\u89c1";
    state = "initial"
  } else if (state === "calling") {
    response = "\u597d\u7684\u6b63\u5728\u6253\u7535\u8bdd\u7ed9 " + user_said;
    state = "initial"
  } else {
    response = "\u6ca1\u542c\u61c2\u4f60\u8bf4\u7684\u662f\u4ec0\u4e48";
  }
...
function speak(text, callback) {
  ...
  u.text = text;
  u.lang = 'zh-CN';
  u.volume = 0.1 //between 0.1
  u.pitch = 1.0 //between 0 and 2
  u.rate = 1.5 //between 0.1 and 5-ish
